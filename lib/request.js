...
class Request {
  ...
  onSuccess(res, buffer) {
    ...
    if (
      this.fetchAll &&
      res.headers['link'] &&
      nextRegex.test(res.headers['link']) &&
      lastRegex.test(res.headers['link'])
    ) {
      let matches = nextRegex.exec(res.headers['link']);
      if (matches && matches[1]) {
        let nextUrl = matches[1];
        if (this.onNextPage && this.onNextPage instanceof Function) {
          let lastPageUrl = lastRegex.exec(res.headers['link'])[1];
          let nextPage = pageNbRegex.exec(nextUrl)[1];
          let lastPage = pageNbRegex.exec(lastPageUrl)[1];
